...
fun_help(){
	echo "\"./dpdk.sh\"                                 help"
    echo "\"./dpdk.sh -c /path/to/dpdk\"                compile dpdk"
	echo "\"./dpdk.sh -s /path/to/dpdk /path/to/sdk\"   build dpdk sdk"
	echo "\"./dpdk.sh -d /path/to/kmods /path/to/sdk\"  build dpdk sdk"
	echo "\"./dpdk.sh -m /path/to/sdk\"                 install kernel module" 
	echo "\"./dpdk.sh -r \"                             remove kernel module"
}

fun_compile(){
	echo " start to compile dpdk..."
	if [ ! -d $1 ]; then
		echo " dpdk directory not exist"
		exit 0
	else
		echo " dpdk directory exist"
	fi
    pushd $1 >> /dev/null
	meson setup build
...
fun_mod(){
    echo " start to install dpdk kernel module..."
	if [ ! -d $1/ko ]; then
		echo " dpdk kmod not exist"
		exit 0
	fi

    if [ ! -d /lib/modules/$(uname -r)/kernel/dpdk ];then
        sudo mkdir /lib/modules/$(uname -r)/kernel/dpdk
    fi

    sudo cp -f  $1//ko/*.ko /lib/modules/$(uname -r)/kernel/dpdk
    sudo depmod
}

fun_rmod(){
    echo " start to remove dpdk kernel module..."
	if [ $(lsmod | grep igb_uio | wc -l) -eq 1 ]; then
	    sudo rmmod igb_uio
		echo " remove igb_uio"
	else
		echo " igb_uio not exist"
	fi
	if [ $(lsmod | grep rte_kni | wc -l) -eq 1 ]; then
	    sudo rmmod rte_kni
		echo " remove rte_kni"
	else
		echo " rte_kni not exist"
	fi
    if [ -d /lib/modules/$(uname -r)/kernel/dpdk ];then
        sudo rm -rf  /lib/modules/$(uname -r)/kernel/dpdk
        echo " delete dpdk kernel module"
	else
		echo " dpdk kernel module not exist"
	fi
	sudo depmod
	echo " done..."
}
...
fun_sdk(){
	...
	if [ ! -d $2 ];then
	   echo " dpdk sdk directory not exist"
	   exit 0
	fi
	...
	#kernel modules
	cp -f $DPDK_BUILD/kernel/linux/kni/*.ko $DPDK_KO
}

fun_igb() {
	if [ ! -d $1/linux ]; then
		echo " dpdk-kmod directory not exist"
		exit 0
	fi
	if [ ! -d $2/ko ];then
	   echo " dpdk sdk directory not exist"
    ...
    cp -f $1/linux/igb_uio/igb_uio.ko $2/ko/igb_uio.ko
}
